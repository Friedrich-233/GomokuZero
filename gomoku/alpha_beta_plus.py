...
class AlphaBetaCM:
    def _init_zobrist(self, n, seed=0):
        rng = np.random.default_rng(seed)
        self.zobrist = rng.integers(1, np.iinfo(np.uint64).max, size=(n, n, 3), dtype=np.uint64)
        self.zobrist_turn = rng.integers(1, np.iinfo(np.uint64).max, size=3, dtype=np.uint64)

    def hash_state(self, game: GomokuEnv):
        n = game.board_size
        if self.zobrist is None or self.zobrist.shape[0] != n:
        ...
        for r in range(n):
            for c in range(n):
                p = int(board[r, c])
                if p:
                    h ^= self.zobrist[r, c, p]
        h ^= self.zobrist_turn[int(game.current_player)]
        return int(h)
    def order_score(self, game: GomokuEnv, action: int) -> float:
        if getattr(self, "_root_player", None) is None:
        ...
        child = game.clone()
        child.step(action)
        score += 0.01 * self.evaluator(child, original_player=self._root_player)

        return score
    
    def generate_candidate_actions(self, game: GomokuEnv, radius: int = 2):
        board = game.board
        n = game.board_size
    ...
    def get_action(self, game: GomokuEnv):
        original_player = game.current_player 
        self._root_player = original_player
        valid_actions = self.generate_candidate_actions(game, radius=2)

        if not valid_actions:
            return None
        valid_actions.sort(key=lambda a: self.order_score(game, a), reverse=True)
    ...
    def minimax(self, game: GomokuEnv, depth: int, alpha: float, beta: float, is_maximizing_player: bool):
        ...
        key = (self.hash_state(game), depth, is_maximizing_player)
        if key in self.tt:
            self.tt_hits += 1
            return self.tt[key]
...
if __name__ == '__main__':
    ...
    player2 = AlphaBetaCM(search_depth=5)
